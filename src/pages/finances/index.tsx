...
import { useRoqClient, useFinanceFindManyWithCount } from 'lib/roq';
...
export function FinanceListPage(props: FinanceListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useFinanceFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.finance.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    { id: 'total_revenue', header: 'total_revenue', accessorKey: 'total_revenue' },
