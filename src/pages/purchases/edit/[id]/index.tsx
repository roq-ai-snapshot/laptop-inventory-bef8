...
import { useRoqClient, usePurchaseFindFirst } from 'lib/roq';
...
function PurchaseEditPage() {
  ...
  const { data, error, isLoading, mutate } = usePurchaseFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: PurchaseInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.purchase.update({
        data: values as RoqTypes.purchase,
        where: {
          id,
        },
      });
