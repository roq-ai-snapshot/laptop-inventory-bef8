...
import { useRoqPlatformUserProfiles, usePurchaseCount, useLaptopCount, useFinanceCount } from 'lib/roq';

import { LaptopListPage as ShowCaseList } from 'pages/laptops';

function DashboardPage() {
  const { session } = useSession();

  const purchaseStat = usePurchaseCount();

  const laptopStat = useLaptopCount();

  const financeStat = useFinanceCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Purchases',
      result: purchaseStat?.data,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`laptops`} passHref legacyBehavior>
                  <Button
